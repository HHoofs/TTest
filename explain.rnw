...
In deze methode gaan we de P-waarde uitrekenen van de t-waarde van het steekproefgemiddelde t.o.v. het populatiegemiddelde berekenen. Hiervoor moeten we eerst de t-waarde weer uitrekenen, welke uiteraard nog steeds dezelfde uitkomst heeft (omdat de gevonden waardes en het populatiegemiddelde niet wijzigen!):
\begin{align}
t=\frac{\bar{X}-\mu}{\frac{s}{\sqrt{N-1}}}= \\
t=\frac{6.65-7}{\frac{2}{sqrt{200-1}}}=\\
t=\frac{-0.35}{2/14.10}=\frac{-0.35}{0.14}=-2.47
